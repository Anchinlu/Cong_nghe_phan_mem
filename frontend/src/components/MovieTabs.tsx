...
const MovieGrid = ({ movies, isUpcoming }: { movies: Movie[], isUpcoming: boolean }) => (
...
export default function MovieTabs({ nowShowingMovies, upcomingMovies }: MovieTabsProps) {
  ...
  return (
    <section className="py-12 bg-black text-white">
      ...
      <div>
        {activeTab === 'now_showing' && <MovieGrid movies={nowShowingMovies} isUpcoming={false} />}
        {activeTab === 'upcoming' && <MovieGrid movies={upcomingMovies} isUpcoming={true} />}
      </div>
    </section>
  );
}
